...
import com.example.cnpjconsulta.dto.CnpjRequest;
...
public class CnpjController {
    ...
    public ResponseEntity<String> consultaCNPJViaPost(@RequestBody CnpjRequest request) {
        try {
            // Chamando o serviço de consulta do CNPJ
            String result = cnpjService.consultaCNPJ(request.getCnpj());
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Erro ao consultar CNPJ: " + e.getMessage());
        }
